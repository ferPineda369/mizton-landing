 ...
 */
function getAllPosts($page = 1, $postsPerPage = 6) {
    $offset = ($page - 1) * $postsPerPage;
    return getBlogPosts($postsPerPage, $offset);
}

/**
 * Obtener posts por categoría con paginación
 */
function getPostsByCategory($category, $page = 1, $postsPerPage = 6) {
    $offset = ($page - 1) * $postsPerPage;
    return getBlogPosts($postsPerPage, $offset, $category);
}

/**
 * Buscar posts
 */
function searchPosts($search, $page = 1, $postsPerPage = 6) {
    $db = getBlogDB();
    $offset = ($page - 1) * $postsPerPage;
    ...
    $stmt = $db->prepare($sql);
    $searchTerm = "%{$search}%";
    $stmt->execute([$searchTerm, $searchTerm, $searchTerm, $postsPerPage, $offset]);
 ...
 */
function getTotalSearchResults($search) {
    $db = getBlogDB();
...
/**
 * Obtener total de posts por categoría
 */
function getTotalPostsByCategory($category) {
    $db = getBlogDB();
    
    $stmt = $db->prepare("SELECT COUNT(*) FROM blog_posts WHERE category = ? AND status = 'published'");
...
function getCategories() {
    ...
    $stmt = $db->prepare("SELECT DISTINCT category FROM blog_posts WHERE status = 'published' ORDER BY category");
    $stmt->execute();
